...
def preprocess_demand(daily_demand_path=r'..\data\Dummy_results_Jan01.csv'):
        
    flight_df = pd.read_csv(daily_demand_path).drop(columns=['Unnamed: 0'])
...
def multiobjective_optimization(flight_df, tablas_parametricas=r'..\data\Tablasoptimizacion.csv', LimiteCajas=1000):
    ...
    ProductoXCaja = pd.read_csv(tablas_parametricas)
    ...
    #Resolviendo problema y guardando solucion
    prob.solve()
    primary_obj = pl.value(prob.objective)

    #Añadir que solo podamos deviar 5% del minimo desperdicio para segunda optimizacion
    prob += pl.lpSum([ProductoXCaja.loc[p,t][0] * C[(t,v)] for (p,t,v) in list(itertools.product(productos, tipos, vuelos))])\
            - pl.lpSum([Demanda.loc[v, p]['value'] for p,v in list(itertools.product(productos, vuelos))]) <= primary_obj * 1.05

    #Funcion objetivo 2: Reduccion de paradas de abastecimiento
    ...
    df_results = df.pivot(index='Vuelo', columns='Tipo de Cajon', values='Cantidad')
