...
ProductoXCaja = pd.read_csv(r'..\data\Tablasoptimizacion.csv').set_index(['producto', 'tipo'])
...
Demanda = pd.melt(flight_df.drop(columns=['Aeronave', 'Capacity', 'Passengers', 'Bookings']),
                  ...
                  var_name='producto').set_index(['LastLoadChance', 'producto'])

#Variables de decision
C = pl.LpVariable.dicts('C', list(itertools.product(tipos, vuelos)), lowBound=0, upBound=4)
R = pl.LpVariable.dicts('R', vuelos, lowBound=0, upBound=4)
...
for (p,v) in list(itertools.product(productos, vuelos)):
    prob += pl.lpSum([ProductoXCaja.loc[p, t][0] * C[(t, v)] for t in tipos]) >= Demanda.loc[(v, p)]['value'][0]
...
prob += pl.lpSum([R[v] for v in vuelos])
